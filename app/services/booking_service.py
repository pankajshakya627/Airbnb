...
class BookingService:
    ...
    async def add_guests(self, booking_id: int, guest_ids: List[int], user: User) -> BookingResponse:
        ...
        result = await self.db.execute(
            select(Guest).where(
                and_(Guest.id.in_(guest_ids), Guest.user_id == user.id)
            )
        )
    ...
    async def get_all_bookings_by_hotel_id(self, hotel_id: int, owner: User) -> List[BookingResponse]:
        ...
        result = await self.db.execute(
            select(Booking).where(Booking.hotel_id == hotel_id)
        )
    ...
    async def _get_room(self, room_id: int, hotel_id: int) -> Room:
        result = await self.db.execute(
            select(Room).where(and_(Room.id == room_id, Room.hotel_id == hotel_id))
        )
    ...
    async def _confirm_inventory(
        ...
        await self.db.execute(
            ...
            .values(
                reserved_count=Inventory.reserved_count - rooms_count,
                book_count=Inventory.book_count + rooms_count
            )
        )
